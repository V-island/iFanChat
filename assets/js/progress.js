...
export default class Progress extends EventEmitter {
    ...
    show(_speed) {
    	let self = this;
    	_speed = _speed !== undefined ? _speed : self.options.speed;
		...
		self.context.arc(self.centerX, self.centerY, self.centerX-1, 0, Math.PI*2, self.options.direction);//用于绘制圆弧context.arc(x坐标，y坐标，半径，起始角度，终止角度，顺时针/逆时针)
		...
		if (self.options.showFont) {
			...
			self.context.strokeText(_speed, self.centerX-25, self.centerY+10);
		}

		// 进度条
		self.context.beginPath(); //路径开始
		self.context.arc(self.centerX, self.centerY, self.centerX-1, -Math.PI/2, -Math.PI/2 + _speed*self.rad, self.options.direction);
